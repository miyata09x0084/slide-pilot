# Multimodal Lab – PDFから動画を自動生成する実験プロダクト
...
## 背景（簡略版）
マルチモーダル生成コストが急速に低下しており、動画/音声も個別最適化が現実的になりつつあります。Multimodal Labはその将来を見越し、PDFから動画への自動変換パイプラインを検証する実験です。より詳細な実装メモや設計は [CLAUDE.md](./CLAUDE.md) を参照してください。
