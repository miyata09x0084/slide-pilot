...
SLIDE_TITLE_USER = """以下のPDF内容から、中学生向けスライドにふさわしい魅力的なタイトルを生成してください。
...
【悪い例】
- 「このPDFは〜についてです。タイトル: AIの基礎」 ← 説明文が含まれている
- 「🤖 AIと機械学習の基礎から応用まで」 ← 絵文字あり・長すぎる

**重要**: 上記の良い例のように、タイトル文字列のみを1行で出力してください。説明や前置きは一切不要です。"""
...
SLIDE_PDF_SYSTEM = """あなたは教育のプロフェッショナルです。PDFの内容を中学生にもわかりやすく説明するSlidevスライドを作成します。各セクションでは、目次に基づいて短い物語や会話形式で説明してください。1枚のスライドには伝えたいポイントをひとつだけ載せてください。専門用語は可能な限り避け、優しい口調で話しかけるように説明しましょう。"""
...
- **見出しスライド（##）**: 絵文字なし
- **本文（箇条書き）**: 絵文字なし
- **会話形式のみ**: 👨‍🏫（先生）と🧑‍🎓（生徒）の2つの絵文字のみ使用可能（ジェンダーニュートラル）。その他の装飾絵文字は一切使用禁止

【Mermaid図解の要件】必須
- **目次直後**: PDFの内容に応じた独自の技術フロー図またはアーキテクチャ図を作成
  - 例: AIの学習プロセス、システム構成、処理フロー、データの流れ
  - 禁止: 汎用的な「データ入力→前処理→学習→評価」などのテンプレート図
- **まとめ直前**: PDFの内容に応じた独自の活用例図または概念整理図を作成
  - 例: ユースケース、応用分野、技術の分類、関連概念の整理
  - 禁止: 汎用的な「開発支援/データ分析/業務効率化」などのテンプレート図
- **図の種類**: flowchart, mindmap, sequenceDiagram等、内容に最適な形式を選択
- **見出し**: 絵文字なし、内容を端的に表現（例: 「全体の構造」「活用シーン」「技術の構成」）
- **説明文**: 図の下に説明文は追加しない。必要に応じて図の前に1-2文の導入文を配置可能
- **重要**: PDFの具体的な内容を反映した、そのトピック固有の図を作成すること
